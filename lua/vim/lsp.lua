--changes from haoliang:
--* localize variables and helpers
--* no insane defaults options and keymaps
--* no features i dont use: semantic_token
--* avoid complicate vim.validate
...
local api = vim.api
local nvim_err_writeln, nvim_buf_get_lines, nvim_command, nvim_buf_get_option, nvim_exec_autocmds = api.nvim_err_writeln, api.nvim_buf_get_lines, api.nvim_command, api.nvim_buf_get_option, api.nvim_exec_autocmds
local uv = vim.loop
...
do
  ...
  ---@param bufnr (number)
  ---@return string
  function buf_get_line_ending(bufnr) return format_line_ending[nvim_buf_get_option(bufnr, "fileformat")] or "\n" end
end
...
local function for_each_buffer_client(bufnr, fn, restrict_client_ids)
  ...
  if not client_ids or tbl_isempty(client_ids) then return end

  if restrict_client_ids and #restrict_client_ids > 0 then
    local filtered_client_ids = {}
    for client_id in pairs(client_ids) do
      if vim.tbl_contains(restrict_client_ids, client_id) then filtered_client_ids[client_id] = true end
    end
  ...
  for client_id in pairs(client_ids) do
    ...
    if client then fn(client, client_id, bufnr) end
  end
end

local validate_client_config
do
  ...
  function validate_client_config(config)
    validate({
      config = { config, "t" },
    })
    validate({
    ...
    if config.offset_encoding then offset_encoding = validate_encoding(config.offset_encoding) end
...
local function buf_get_full_text(bufnr)
  --todo: this ending has no meaning
  local line_ending = buf_get_line_ending(bufnr)
  local text = table.concat(nvim_buf_get_lines(bufnr, 0, -1, true), line_ending)
  --todo: avoid realloc string
  if nvim_buf_get_option(bufnr, "eol") then text = text .. line_ending end
  return text
end
...
do
  ...
  local function group_key(group)
    if group.sync_kind == protocol.TextDocumentSyncKind.Incremental then return tostring(group.sync_kind) .. "\0" .. group.offset_encoding end
  ...
  local state_by_group = setmetatable({}, {
    __index = function(tbl, k) return rawget(tbl, group_key(k)) end,
    __newindex = function(tbl, k, v) rawset(tbl, group_key(k), v) end,
  })
  ...
  local function get_group(client)
    ...
    local change_capability = vim.tbl_get(client.server_capabilities or {}, "textDocumentSync", "change")
    local sync_kind = change_capability or protocol.TextDocumentSyncKind.None
    if not allow_inc_sync and change_capability == protocol.TextDocumentSyncKind.Incremental then sync_kind = protocol.TextDocumentSyncKind.Full end
    return {
      sync_kind = sync_kind,
  ...
  local function incremental_changes(state, encoding, bufnr, firstline, lastline, new_lastline)
    ...
    local line_ending = buf_get_line_ending(bufnr)
    local incremental_change = sync.compute_diff(state.lines, curr_lines, firstline, lastline, new_lastline, encoding, line_ending)
  ...
  function changetracking.init(client, bufnr)
    ...
    if buf_state then
      ...
      if group.sync_kind == protocol.TextDocumentSyncKind.Incremental then buf_state.lines = nvim_buf_get_lines(bufnr, 0, -1, true) end
    end
  end
  ...
  local function next_debounce(debounce, buf_state)
    if debounce == 0 then return 0 end
    local ns_to_ms = 0.000001
    if not buf_state.last_flush then return debounce end
    local now = uv.hrtime()
  ...
  local function send_changes(bufnr, sync_kind, state, buf_state)
    if not buf_state.needs_flush then return end
  ...
  function changetracking.send_changes(bufnr, firstline, lastline, new_lastline)
    ...
    for _, group in pairs(groups) do
      local state = state_by_group[group]
      if not state then error(string.format("changetracking.init must have been called for all LSP clients. group=%s states=%s", vim.inspect(group), vim.inspect(vim.tbl_keys(state_by_group)))) end
      local buf_state = state.buffers[bufnr]
      ...
      if group.sync_kind == protocol.TextDocumentSyncKind.Incremental then
        ...
        local changes = incremental_changes(buf_state, group.offset_encoding, bufnr, firstline, lastline, new_lastline)
...
local function text_document_did_open_handler(bufnr, client)
  ...
  if not vim.tbl_get(client.server_capabilities, "textDocumentSync", "openClose") then return end
...
api.nvim_create_autocmd("VimLeavePre", {
  ...
  callback = function()
    ...
    if tbl_isempty(active_clients) then return end
    ...
    local function check_clients_closed()
      ...
      for client_id, _ in pairs(active_clients) do
        if timeouts[client_id] ~= nil and timeouts[client_id] > 0 then return false end
      end
      return true
    end
    ...
    if send_kill then
      if not vim.wait(max_timeout, check_clients_closed, poll_time) then
        for client_id, client in pairs(active_clients) do
          if timeouts[client_id] ~= nil then client.stop(true) end
        end
      end
    end
  end,
...
do
  ...
  function lsp._cmd_parts(input)
    ...
    for i, v in ipairs(input) do
      ...
      if i > 1 then table.insert(cmd_args, v) end
    end
    return cmd, cmd_args
  end
  ...
  function lsp.start(config, opts)
    opts = opts or {}
    local reuse_client = opts.reuse_client or function(client, conf) return client.config.root_dir == conf.root_dir and client.name == conf.name end
    config.name = config.name
    if not config.name and type(config.cmd) == "table" then config.name = config.cmd[1] and vim.fs.basename(config.cmd[1]) or nil end
  ...
  do
    ...
    function lsp.start_client(config)
      ...
      local function resolve_handler(method) return handlers[method] or default_handlers[method] end
      ...
      local function is_empty_or_default(bufnr, option)
        if vim.bo[bufnr][option] == "" then return true end
        ...
        if #scriptinfo ~= 1 then return false end
      ...
      local function set_defaults(client, bufnr)
        ...
        if capabilities.definitionProvider and is_empty_or_default(bufnr, "tagfunc") then vim.bo[bufnr].tagfunc = "v:lua.vim.lsp.tagfunc" end
        if capabilities.completionProvider and is_empty_or_default(bufnr, "omnifunc") then vim.bo[bufnr].omnifunc = "v:lua.vim.lsp.omnifunc" end
        if capabilities.documentRangeFormattingProvider and is_empty_or_default(bufnr, "formatprg") and is_empty_or_default(bufnr, "formatexpr") then vim.bo[bufnr].formatexpr = "v:lua.vim.lsp.formatexpr()" end
      end
      ...
      local function unset_defaults(bufnr)
        if vim.bo[bufnr].tagfunc == "v:lua.vim.lsp.tagfunc" then vim.bo[bufnr].tagfunc = nil end
        if vim.bo[bufnr].omnifunc == "v:lua.vim.lsp.omnifunc" then vim.bo[bufnr].omnifunc = nil end
        if vim.bo[bufnr].formatexpr == "v:lua.vim.lsp.formatexpr()" then vim.bo[bufnr].formatexpr = nil end
      end
      ...
      function dispatch.on_exit(code, signal)
        if config.on_exit then pcall(config.on_exit, code, signal, client_id) end

        local client = active_clients[client_id] and active_clients[client_id] or uninitialized_clients[client_id]

        for bufnr, client_ids in pairs(all_buffer_active_clients) do
          if client_ids[client_id] then
            vim.schedule(function()
              if client and client.attached_buffers[bufnr] then nvim_exec_autocmds("LspDetach", {
                buffer = bufnr,
                modeline = false,
              ...
              if vim.tbl_isempty(client_ids) then unset_defaults(bufnr) end
            end)
          end
        end
        ...
        vim.schedule(function()
          ...
          if client then changetracking.reset(client) end
      ...
      local function initialize()
        ...
        rpc.request("initialize", initialize_params, function(init_err, result)
          ...
          if next(config.settings) then client.notify("workspace/didChangeConfiguration", { settings = config.settings }) end

          if config.on_init then
            local status, err = pcall(config.on_init, client, result)
            if not status then pcall(handlers.on_error, lsp.client_errors.ON_INIT_CALLBACK_ERROR, err) end
          end
          ...
          for bufnr, client_ids in pairs(all_buffer_active_clients) do
            if client_ids[client_id] then client._on_attach(bufnr) end
          end
        end)
      end
      ...
      function client.request(method, params, handler, bufnr)
        if not handler then handler = assert(resolve_handler(method), string.format("not found: %q request handler for client %q.", method, client.name)) end
        -- Ensure pending didChange notifications are sent so that the server doesn't operate on a stale state
        changetracking.flush(client, bufnr)
        bufnr = resolve_bufnr(bufnr)
        local _ = log.debug() and log.debug(log_prefix, "client.request", client_id, method, params, handler, bufnr)
        local success, request_id = rpc.request(method, params, function(err, result) handler(err, result, { method = method, client_id = client_id, bufnr = bufnr, params = params }) end, function(request_id)
      ...
      function client.notify(method, params)
        if method ~= "textDocument/didChange" then changetracking.flush(client) end
      ...
      function client.is_stopped() return rpc.is_closing() end
      ...
      function client._on_attach(bufnr)
        ...
        vim.schedule(function()
          if vim.tbl_get(client.server_capabilities, "semanticTokensProvider", "full") then semantic_tokens.start(bufnr, client.id) end
        end)
  ...
  do
    ...
    local function text_document_did_change_handler(_, bufnr, changedtick, firstline, lastline, new_lastline)
      ...
      if tbl_isempty(all_buffer_active_clients[bufnr] or {}) then return true end
    ...
    local function text_document_did_save_handler(bufnr)
      ...
      for_each_buffer_client(bufnr, function(client)
        ...
        if save_capability then
          local included_text
          if type(save_capability) == "table" and save_capability.includeText then included_text = text(bufnr) end
    ...
    function lsp.buf_attach_client(bufnr, client_id)
      ...
      if not buffer_client_ids then
        ...
        api.nvim_create_autocmd("BufWritePre", {
          ...
          callback = function(ctx)
            for_each_buffer_client(ctx.buf, function(client)
              ...
              if vim.tbl_get(client.server_capabilities, "textDocumentSync", "willSave") then client.notify("textDocument/willSave", params) end
              if vim.tbl_get(client.server_capabilities, "textDocumentSync", "willSaveWaitUntil") then
                local result, err = client.request_sync("textDocument/willSaveWaitUntil", params, 1000, ctx.buf)
        ...
        api.nvim_create_autocmd("BufWritePost", {
          ...
          callback = function(ctx) text_document_did_save_handler(ctx.buf) end,
        })
        ...
        api.nvim_buf_attach(bufnr, false, {
          ...
          on_reload = function()
            ...
            for_each_buffer_client(bufnr, function(client, _)
              ...
              if vim.tbl_get(client.server_capabilities, "textDocumentSync", "openClose") then client.notify("textDocument/didClose", params) end
              text_document_did_open_handler(bufnr, client)
            end)
          end,
          on_detach = function()
      ...
      if client then client._on_attach(bufnr) end
      return true
    end
  end
  ...
  function lsp.buf_detach_client(bufnr, client_id)
    ...
    if not client or not client.attached_buffers[bufnr] then
      vim.notify(string.format("Buffer (id: %d) is not attached to client (id: %d). Cannot detach.", bufnr, client_id))
      return
    end
    ...
    if #vim.tbl_keys(all_buffer_active_clients[bufnr]) == 0 then all_buffer_active_clients[bufnr] = nil end
  ...
  function lsp.buf_is_attached(bufnr, client_id) return (all_buffer_active_clients[resolve_bufnr(bufnr)] or {})[client_id] == true end
  ...
  function lsp.get_client_by_id(client_id) return active_clients[client_id] or uninitialized_clients[client_id] end
  ...
  function lsp.get_active_clients(filter)
    ...
    for client_id in pairs(t) do
      local client = active_clients[client_id]
      if client and (filter.id == nil or client.id == filter.id) and (filter.name == nil or client.name == filter.name) then clients[#clients + 1] = client end
    end
    return clients
  end
  ...
  function lsp.buf_request_all(bufnr, method, params, callback)
    ...
    local set_expected_result_count = once(function()
      for_each_buffer_client(bufnr, function(client)
        if client.supports_method(method) then expected_result_count = expected_result_count + 1 end
      end)
    end)
    ...
    local function _sync_handler(err, result, ctx)
      ...
      if result_count >= expected_result_count then callback(request_results) end
    end
  ...
  function lsp.buf_request_sync(bufnr, method, params, timeout_ms)
    ...
    local cancel = lsp.buf_request_all(bufnr, method, params, function(it) request_results = it end)

    local wait_result, reason = vim.wait(timeout_ms or 1000, function() return request_results ~= nil end, 10)
  ...
  function lsp.formatexpr(opts)
    ...
    for _, client in pairs(lsp.get_active_clients({ bufnr = bufnr })) do
      if client.supports_method("textDocument/rangeFormatting") then
        ...
        local response = client.request_sync("textDocument/rangeFormatting", params, timeout_ms, bufnr)
  ...
  function lsp.tagfunc(...) return require("vim.lsp.tagfunc")(...) end

  ---Checks whether a client is stopped.
  ---
  ---@param client_id (integer)
  ---@return boolean stopped true if client is stopped, false otherwise.
  function lsp.client_is_stopped(client_id) return active_clients[client_id] == nil end
  ...
  function lsp.get_log_path() return log.get_filename() end
  ...
  ---               end)
  ---             </pre>
  function lsp.for_each_buffer_client(bufnr, fn) return for_each_buffer_client(bufnr, fn) end

  --- Function to manage overriding defaults for LSP handlers.
  ---@param handler (function) See |lsp-handler|
  ---@param override_config (table) Table containing the keys to override behavior of the {handler}
  function lsp.with(handler, override_config)
    return function(err, result, ctx, config) return handler(err, result, ctx, vim.tbl_deep_extend("force", config or {}, override_config)) end
  end
  ...
  function lsp._with_extend(name, options, user_config)
    ...
    for k, v in pairs(user_config) do
      if options[k] == nil then error(debug.traceback(string.format("Invalid option for `%s`: %s. Valid options are:\n%s", name, k, vim.inspect(vim.tbl_keys(options))))) end

      resulting_config[k] = v
    end

    for k, v in pairs(options) do
      if resulting_config[k] == nil then resulting_config[k] = v end
    end
